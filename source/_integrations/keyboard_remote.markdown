...
automation:
  ...
  trigger:
    ...
    event_data:
      device_descriptor: "/dev/input/event0"
      name: space # inspect event log to obtain desired name
...
`name` (or `key_code`) should be set to the code generated by the key press, otherwise the trigger will fire on every key press.
