...
The `evdev` package is used to interface with the keyboard and thus this is Linux only. It also means you can't use your normal keyboard for this because `evdev` will grap all input from it.
...
Events of type `keyboard_remote_command_received` have the following fields:

Field | Example | Description
--- | --- | ---
`key_code` | 57 | The code of the key pressed. Use the event log in Developer Tools to get a key code.
`key_name` | space | The name of the key pressed. Use the event log in Developer Tools to get a key name.
...
And an example automation rule to breathe life into it:
...
automation:
  ...
  trigger:
    ...
    event_data:
      device_descriptor: "/dev/input/event0"
      key_name: space # inspect event log to obtain desired name
...
`key_name` (or `key_code`) should be set to the code generated by the key press, otherwise the trigger will fire on every key press.
